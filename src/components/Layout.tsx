...
import { LayoutDashboard, Wallet, Gift, ShieldAlert, LogOut, Users, Moon, Sun, Globe, Coins, Bell } from 'lucide-react';
...
export default function Layout({ children, activeTab, onTabChange }: {
  ...
  const menuItems = [
    ...
    ...(user?.isAdmin ? [{ id: 'admin', label: t('admin'), icon: ShieldAlert }] : []),
  ];
      ...
      <header className="sticky top-0 z-40 bg-[var(--card-bg)] border-b border-[var(--card-border)]" style={{ boxShadow: '0 1px 12px rgba(0,0,0,0.05)' }}>
        <div className="max-w-7xl mx-auto flex justify-between items-center px-5 py-4">
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: '#6C3EF4' }}>
              <Coins size={18} color="white" />
            </div>
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 18, color: '#6C3EF4', letterSpacing: '-0.5px' }}>
              EARNIFY
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="tap-effect w-9 h-9 rounded-xl flex items-center justify-center bank-card relative">
              <Bell size={16} color="var(--muted)" />
            </button>
            {/* Avatar */}
            <div className="w-9 h-9 rounded-xl flex items-center justify-center font-bold text-sm text-white ml-1" style={{ background: '#6C3EF4' }}>
              {initials}
            </div>
          </div>
        </div>
        ...
        <aside className="hidden md:flex w-64 h-[calc(100vh-65px)] flex-col sticky top-[65px] p-6 gap-2">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                style={{
                  background: active ? '#6C3EF4' : 'transparent',
          ...
          <div className="mt-auto">
            <div className="bank-card p-4 rounded-2xl mb-3">
              <p style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 2 }}>Balance</p>
              <p style={{ fontSize: 22, fontWeight: 800, fontFamily: 'var(--font-display)', color: '#6C3EF4' }}>
                ${Number(user?.balance || 0).toFixed(2)}
              </p>
            </div>
            <button
              onClick={logout}
              className="tap-effect flex items-center gap-3 px-4 py-3 rounded-2xl w-full text-sm font-semibold transition-all"
              style={{ color: '#EF4444' }}
            >
              <LogOut size={18} />
              {t('logout')}
          ...
          {menuItems.slice(0, 4).map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => onTabChange(item.id)}
                className="tap-effect flex flex-col items-center gap-1 py-2 px-3"
              >
                <div className="w-10 h-10 rounded-2xl flex items-center justify-center transition-all"
                  style={{ background: active ? '#EDE9FE' : 'transparent' }}>
                  <item.icon size={20} strokeWidth={active ? 2.5 : 1.8} color={active ? '#6C3EF4' : 'var(--muted)'} />
                </div>
                <span style={{ fontSize: 9, fontWeight: active ? 700 : 500, color: active ? '#6C3EF4' : 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  {item.label}
                </span>
              </button>
            );
